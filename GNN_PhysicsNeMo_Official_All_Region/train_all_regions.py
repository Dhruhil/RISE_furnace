...
def train_one_epoch(model, loader, optimizer, device, cfg, Y_std, lam=0.001):
    model.train()
    total_loss = 0.0
    n = 0
    for batch in loader:
        ...
        # Physics loss
        if lam > 1e-10:
            dT_std  = (float(batch.dT_std[0])
                       if hasattr(batch.dT_std, "__len__")
                       else float(batch.dT_std))
            dT_mean = (float(batch.dT_mean[0])
                       if hasattr(batch.dT_mean, "__len__")
                       else float(batch.dT_mean))
            L_phys = physics_loss_allregions(
                pred, batch, dT_std, dT_mean, str(device)
            )
