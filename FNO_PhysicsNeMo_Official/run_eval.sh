#!/bin/bash
# FNO Rollout Evaluation — real per-region accuracy
...
echo "=== Writing evaluation/evaluate_fno3d.py ==="
mkdir -p evaluation
...
echo "  OK: evaluation/evaluate_fno3d.py created"

# Create SLURM script for evaluation
cat > run_eval_fno.sh << 'SHEOF'
#!/bin/bash
...
apptainer exec --nv /mimer/NOBACKUP/groups/revar/physicsnemo_25.06.sif \
  python -u evaluation/evaluate_fno3d.py --device cuda --n_sims 5
SHEOF

echo "  OK: run_eval_fno.sh created"

echo ""
echo "=== DONE ==="
echo ""
echo "  Submit evaluation:"
echo "    sbatch run_eval_fno.sh"
echo ""
echo "  Output shows:"
echo "    Region       | Cells | P1 MAE  | P2 MAE"
echo "    steel_cylinder |  450 | XX.XXK  | XX.XXK"
echo "    inner_box      | 3000 |  X.XXK  |  X.XXK"
echo "    outer_box      | 5000 |  X.XXK  |  X.XXK"
echo "    heater_1       |  200 |  0.00K  |  0.00K (BC)"
echo ""
echo "  P1 = 0-3200s (training window)"
echo "  P2 = 3200-4000s (extrapolation — the real test)"
echo "  steel_cylinder P1 and P2 are the KEY thesis numbers"
