#!/bin/bash
# FNO Plotting — training curves + rollout plots for thesis
...
echo "=== Creating FNO plot scripts ==="
mkdir -p outputs/plots

cat > plots_fno.py << 'PYEOF'
"""
FNO Thesis Plots:
...
echo "  OK: plots_fno.py created"

# Create SLURM script for plotting (needs GPU for rollout plot)
cat > run_plots.sh << 'SHEOF'
#!/bin/bash
#SBATCH --job-name=fno_plots
#SBATCH --account=NAISS2026-4-525
#SBATCH --partition=alvis
#SBATCH --output=outputs/logs/plots_%j.log
#SBATCH --error=outputs/logs/plots_err_%j.log
...
echo "  OK: run_plots.sh created"
echo ""
echo "=== DONE ==="
echo ""
echo "  Submit plotting job:"
echo "    sbatch run_plots.sh"
echo ""
echo "  Plots saved to outputs/plots/:"
echo "    fno_training_curves.png    — loss, physics, curriculum"
echo "    fno_rollout_comparison.png — predicted vs ground truth"
echo "    fno_speed_comparison.png   — OpenFOAM vs FNO speed"
echo ""
echo "  Download plots:"
echo "    scp dhruhil@alvis1:/mimer/NOBACKUP/groups/revar/FNO_PhysicsNeMo_Official/outputs/plots/*.png ."
