    ...
    for batch in loader:
        ...
        if w2 > 1e-6:
            ...
            batch2 = Data(
                ...
                is_heater=batch.is_heater, T_current=T_pred1,
                T_set_raw=batch.T_set_raw,
            )
            pred2 = model(batch2)
...
def evaluate(model, loader, device, lam=0.0):
    ...
    for batch in loader:
        ...
        rw = get_region_weights(batch)
        loss_data = weighted_mse(pred_masked, target_masked, rw)
        total_data += loss_data.item()

        if lam > 1e-6:
            L_phys = physics_loss_unified(pred, batch, T_std_ds, T_mean)
            total_phys += L_phys.item()
        n += 1
    ...
    # Per-region MAE
    if all_steel_pred:
        sp = np.concatenate(all_steel_pred)
        st = np.concatenate(all_steel_true)
        m["steel_mae"] = float(np.mean(np.abs(sp - st)))
    for rname in ["steel", "air", "outer"]:
        if region_preds[rname]:
            rp = np.concatenate(region_preds[rname])
            rt = np.concatenate(region_trues[rname])
            m[f"{rname}_mae"] = float(np.mean(np.abs(rp - rt)))
...
def main():
    ...
    h5 = cfg.all_regions_dataset_path
