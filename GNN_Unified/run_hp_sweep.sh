...
echo "=== HP Sweep: 3 LR x 3 lam, 10 epochs each, batch=2 ===" | tee $SUMMARY
echo "Start: $(date)" | tee -a $SUMMARY
echo "Sweep dir: $SWEEP_DIR" | tee -a $SUMMARY
  ...
  for LAM in "${LAMS[@]}"; do
    ...
    apptainer exec --nv --cleanenv \
      ...
      python -u train_unified.py \
        --epochs 3 --lr $LR --lam $LAM --batch 2 \
        2>&1 | tee $RUN_LOG

    LAST=$(grep -E "^\s*[0-9]+\s*\|" $RUN_LOG | tail -1)
